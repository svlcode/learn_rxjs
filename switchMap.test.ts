...
it('switchMap to convert a stream of combineLatest latest to a single output value in order to simulate c = a + b', (done) => {
  ...
  const sum = combineLatest([a, b]).pipe(switchMap(([e1, e2]) => of(e1 + e2)));
